...
if ($_SERVER["REQUEST_METHOD"] == "POST" and isset($_POST['login1'])) 
  ...
  if (!empty($email) && !empty($pass)) {
      ...
      $myfile = fopen("Accounts/".$email.".csv", "r") or die("This Account does not exist");
	  $acc= fgetcsv($myfile);
	...
	if ($low_email == $acc[1] && $pass == $acc[2]){
		$trackfile = fopen("Accounts/Tracking/".$email.".csv","a+") or die("This Account does not exist");
	  ...
	  echo "Please complete all the feilds";
